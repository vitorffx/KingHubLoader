...
do
	...
	function KingLib:Load(Config)
		print('KingLib ! Loader Loaded')
		local LOADER = Loader
		LOADER.Enabled = true
		LOADER.Parent = coregui

		-- PreLoad
		Config.Name = Config.Name or 'KingLib™'
		...
		local function load()
			...
			if Config.ConfigurationSaving and Config.ConfigurationSaving.Enabled then
				...
				if isfolder and not isfolder(folderName) then
					...
					if not success then
						warn("KingLib ! Failed to create folder:", err)
					end
				end
				...
				if not success then
					warn("KingLib ! File write/read blocked or failed:", err)
				end
			end
			...
			if not successLibrary or not Library then
				warn("KingLib ! Failed to load Library.")
				return
			end
			...
			task.spawn(function()
				while Library and Library.Parent do
					...
					if Library.Parent ~= coregui then
						warn("KingLib ! UI moved. Restoring...")
						pcall(function()
							Library.Parent = coregui
						end)
					end
			...
			task.spawn(function()
				...
				warn("KingLib ! UI removed. Reinjection in progress...")
			...
			if Config.AutoJoinDiscord and Config.AutoJoinDiscord.Enabled then
				...
				warn("KingLib ! Discord AutoJoin enabled. Preparing to send invite:", inviteCode)
				...
				if isfolder and not isfolder(discordFolder) then
					...
					if folderSuccess then
						print("KingLib ! Created DiscordInvites folder at:", discordFolder)
					else
						warn("KingLib ! Failed to create DiscordInvites folder:", folderErr)
					end
				end

				local shouldPrompt = true
				if isfile and discordConfig.RememberJoins and isfile(inviteFilePath) then
					print("KingLib ! Discord invite '" .. inviteCode .. "' already joined. Skipping prompt.")
					shouldPrompt = false
				end
				...
				if shouldPrompt then
					if request then
						...
						if reqSuccess then
							print("KingLib ! Discord invite sent successfully to:", inviteCode)
						else
							warn("KingLib ! Failed to send Discord invite:", reqErr)
						end
					else
						warn("KingLib ! Request function not available — cannot send Discord invite.")
					end
					...
					if discordConfig.RememberJoins then
						...
						if writeSuccess then
							print("KingLib ! Logged Discord invite join for:", inviteCode)
						else
							warn("KingLib ! Failed to write join log for invite:", writeErr)
						end
					end
				end
			end
		...
		if FlagValue ~= nil then
			task.spawn(function()
				...
				elseif Flag.StarterValue then
					...
					warn("KingLib ! Unsupported flag type or missing setter for flag: " .. FlagName)
				end
			end)
		end
		...
		function KingLib:LoadSaveConfig()
			...
			if KingLib.ConfigEnabled then
				...
				elseif not success then
					warn("KingLib Configurations Error ! " .. tostring(result))
				end
			end
		end
