...
-- [ THEME MANAGEMENT ]
function KingLib:UpdateTheme(Config)
...
function KingLib:DeepMerge(target, source)
...
function KingLib:getdark(Color, val, mode)
...
function KingLib:HidePlaceHolder(instance, placeholder, recursive)
...
function KingLib:AddConnection(Type, Callback)
	...
	KingLib.Connections = KingLib.Connections or {}
	table.insert(KingLib.Connections, ConnectionData)
	...
	local function Disconnect()
		...
		for i = #KingLib.Connections, 1, -1 do
			if KingLib.Connections[i] == ConnectionData then
				table.remove(KingLib.Connections, i)
				break
			end
		end
	end
	...
	task.spawn(function()
		task.wait(10)
		for i = #KingLib.Connections, 1, -1 do
			if not KingLib.Connections[i].Connection.Connected then
				table.remove(KingLib.Connections, i)
			end
		end
	end)
...
function KingLib:MakeResizable(Dragger, Object, MinSize, Callback, LockAspectRatio)
	...
	KingLib:AddConnection(Dragger.InputBegan, onInputBegan)
	KingLib:AddConnection(userInput.InputChanged, onInputChanged)
	KingLib:AddConnection(Dragger.InputEnded, onInputEnded)

end

local loadTweens = {}

function KingLib:registerLoadTween(object, properties, initialState, tweenInfo)
...
function KingLib:resetToInitialState(animated, resetTweenInfo)
...
function KingLib:replayLoadTweens(targetObject)
	KingLib:resetToInitialState(false)
...
function KingLib:removeLoadTween(object)
...
function KingLib:ColorPack(color)
...
function KingLib:ColorUnpack(color)
...
function KingLib:updateLayout(container, spacing)
...
function KingLib:AddDrag(Object, Main, ConstrainToParent)
	...
	KingLib:AddConnection(Object.InputBegan, function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			dragging = true
	...
	KingLib:AddConnection(userInput.InputChanged, function(input)
	...
	KingLib:AddConnection(Object.InputEnded, function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			dragging = false
		end
	end)
...
function KingLib:Notify(Notification)
	task.spawn(function()
...
do
	...
	function KingLib:Load(Config)
		print('KingLib ã€¡ Loader Loaded')
		local LOADER = Loader
		LOADER.Enabled = true
		LOADER.Parent = coregui

		-- PreLoad
		Config.Name = Config.Name or 'KingLibâ„¢'
		Config.Logo = Config.Logo or 'rbxassetid://14554547135'
		Config.ConfigFolder = Config.ConfigFolder or 'KingLib'
		Config.Status = Config.Status or false
		Config.Accent = Config.Accent or KingLib.theme.Accent
		Config.HitBox = Config.HitBox or KingLib.theme.HitBox
		...
		local LoaderConfig = {
			...
			Accent = Config.Accent or KingLib.theme.Accent;
			Hitbox = Config.HitBox or KingLib.theme.HitBox;
			Socials = {}
		}
		...
		if typeof(Config.Socials) == "table" and #Config.Socials > 0 then
			...
			KingLib:HidePlaceHolder(LOADER.load.social.largeblock, 'largesocial')
			KingLib:HidePlaceHolder(LOADER.load.social, 'little')
			KingLib:HidePlaceHolder(LOADER.load.social.little, 'smallblock1')
			KingLib:HidePlaceHolder(LOADER.load.social.little, 'smallblock2')
			...
			if largeCount > 1 or smallCount > 2 then
				warn("[KingLib] Only 1 Large block and 2 Small blocks are allowed in Socials.")
		...
		KingLib.theme.Accent = Config.Accent;
		KingLib.theme.HitBox = Config.HitBox;
		LOADER.load.info.build.Text = KingLib.Build
		...
		local function load()
			...
			if Config.ConfigurationSaving and Config.ConfigurationSaving.Enabled then
				local folderName = Config.ConfigurationSaving.FolderName or "KingLibSec"
				local fileName = Config.ConfigurationSaving.FileName or "default_config"
				local fullPath = folderName .. "/" .. fileName .. ".lua"

				KingLib.ConfigEnabled = true
				KingLib.ConfigFolder = folderName  
				KingLib.ConfigFile = fileName     
				...
				if isfolder and not isfolder(folderName) then
					...
					if not success then
						warn("KingLib ã€¡ Failed to create folder:", err)
					end
				end
				...
				if not success then
					warn("KingLib ã€¡ File write/read blocked or failed:", err)
				end
			end


			TweenWorkLabel(1,'rbxassetid://105810189969774', 'Cleaning Things Up...')
			local UI_TAG = "KingLibUILoader"
			local MARKER_NAME = "KingLibUIDetector"
			local INTERNAL_UUID = ("KingLib-" .. tostring(game.JobId):gsub("-", "") .. tostring(tick())):gsub("%.", "")
			...
			if not successLibrary or not Library then
				warn("KingLib ã€¡ Failed to load Library.")
				return
			end
			...
			task.spawn(function()
				while Library and Library.Parent do
					...
					if Library.Parent ~= coregui then
						warn("KingLib ã€¡ UI moved. Restoring...")
						pcall(function()
							Library.Parent = coregui
						end)
					end
			...
			task.spawn(function()
				...
				warn("KingLib ã€¡ UI removed. Reinjection in progress...")
			...
			if Config.AutoJoinDiscord and Config.AutoJoinDiscord.Enabled then
				...
				local rootFolder = Config.ConfigurationSaving and Config.ConfigurationSaving.FolderName or "KingLibSec"
				...
				warn("KingLib ã€¡ Discord AutoJoin enabled. Preparing to send invite:", inviteCode)
				...
				if isfolder and not isfolder(discordFolder) then
					...
					if folderSuccess then
						print("KingLib ã€¡ Created DiscordInvites folder at:", discordFolder)
					else
						warn("KingLib ã€¡ Failed to create DiscordInvites folder:", folderErr)
					end
				end

				local shouldPrompt = true
				if isfile and discordConfig.RememberJoins and isfile(inviteFilePath) then
					print("KingLib ã€¡ Discord invite '" .. inviteCode .. "' already joined. Skipping prompt.")
					shouldPrompt = false
				end
				...
				if shouldPrompt then
					if request then
						...
						if reqSuccess then
							print("KingLib ã€¡ Discord invite sent successfully to:", inviteCode)
						else
							warn("KingLib ã€¡ Failed to send Discord invite:", reqErr)
						end
					else
						warn("KingLib ã€¡ Request function not available â€” cannot send Discord invite.")
					end
					...
					if discordConfig.RememberJoins then
						...
						if writeSuccess then
							print("KingLib ã€¡ Logged Discord invite join for:", inviteCode)
						else
							warn("KingLib ã€¡ Failed to write join log for invite:", writeErr)
						end
					end
				end
			end
...
function LoadConfig(Configuration)
	...
	for FlagName, Flag in pairs(KingLib.Flags) do
		...
		if FlagValue ~= nil then
			task.spawn(function()
				...
				if Flag.Set then
					Flag:Set(Flag.Type == "ColorPicker" and KingLib:ColorUnpack(FlagValue) or FlagValue)
				elseif Flag.Type == "ColorPicker" and Flag.Color then
					Flag.Color = KingLib:ColorUnpack(FlagValue)
					changed = true
				...
				elseif Flag.StarterValue then
					...
					warn("KingLib ã€¡ Unsupported flag type or missing setter for flag: " .. FlagName)
				end
			end)
		end
...
function SaveConfig()
	if not KingLib.ConfigEnabled then return end

	local Data = {}
	for i, v in pairs(KingLib.Flags) do
		if v.Type == "ColorPicker" then
			Data[i] = KingLib:ColorPack(v.Color)
		else
	...
	if writefile then
		local filePath = string.format("%s/%s.lua", KingLib.ConfigFolder, KingLib.ConfigFile)
...
-- [Remove PlaceHolders]
KingLib:HidePlaceHolder(TABS, 'Tb')
KingLib:HidePlaceHolder(PAGES, 'Page')


--[INITIALIZATION]

function KingLib:Init(library)
	...
	local Data = {
		Title = library.Title or "KingLib",
	...
	KingLib:AddDrag(TOPBAR, WINDOW, true)
	KingLib:MakeResizable(WINDOW.resize, WINDOW, Vector2.new(654, 428))

	-- [Initial Transparency Setup]
	TOPBAR.Title.TextTransparency = 1
	TOPBAR.Title.Sub.TextTransparency = 1

	-- KingLib Connection (Coming Soon)
	SetUserInfo()
	...
	KingLib:AddConnection(KingLib.Comms.Event, function(p, color)
	...
	do
		...
		function SettingsElements:GetSettingsElements()
			...
			function element:ColorPicker(ColorPicker, Parent)
				...
				KingLib:AddConnection(SVPicker.InputBegan, function(input)
				...
				KingLib:AddConnection(SVPicker.InputEnded, function(i)
				...
				KingLib:AddConnection(HUESlider.InputBegan, function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 then
				...
				KingLib:AddConnection(HUESlider.InputEnded, function(i)
				...
				if ColorPickerData.SFlag then
					KingLib.SettingsFlags[ColorPickerData.SFlag] = ColorPickerData
				end
		...
		--[[Slider.slide.slideframe.BackgroundColor3 = KingLib.theme.HitBox
		Slider.slide.slideframe.shadowHolder.ambientShadow.ImageColor3 = KingLib.theme.HitBox
		Slider.slide.slideframe.shadowHolder.penumbraShadow.ImageColor3 = KingLib.theme.HitBox
		Slider.slide.slideframe.shadowHolder.umbraShadow.ImageColor3 = KingLib.theme.HitBox]]
		...
		--[[KingLib:AddConnection(KingLib.Comms.Event, function(p, color)
			if p == 'HitBox' then
				...
				Slider.slide.slideframe.shadowHolder.umbraShadow.ImageColor3 = color
					...
					if Options.SFlag then
						KingLib.SettingsFlags[Options.SFlag] = Options
					end
			...
			function element:Toggle(Toggle, Parent)
				...
				local function UpdateToggleUI(state)
					local targetColor = state and KingLib.theme.HitBox or Color3.fromRGB(20, 20, 20)
				...
				if ToggleData.SFlag then
					KingLib.SettingsFlags[ToggleData.SFlag] = ToggleData
				end
		...
		element:ColorPicker({
			Title = 'Drop Shadow',
			Linkable = true,
			Color = KingLib.theme.DropShadow;
			CallBack = function(c)
				KingLib:UpdateTheme({
					['DropShadow'] = c
				})
		...
		function KingLib:SaveSettingsConfig()
			local Data = {}


			for flag, v in pairs(KingLib.SettingsFlags or {}) do

				if v.Type == "ColorPicker" and v.Color then
					Data[flag] = KingLib:ColorPack(v.Color)
			...
			local path = string.format("%s/SettingsConfig.lua", KingLib.ConfigFolder)
		...
		function KingLib:LoadSettingsConfig()
			local path = string.format("%s/SettingsConfig.lua", KingLib.ConfigFolder)
			...
			for flag, val in pairs(data) do
				local setting = KingLib.SettingsFlags and KingLib.SettingsFlags[flag]
				if setting then
					if setting.Set then
						setting:Set(val)
					elseif setting.Type == "ColorPicker" and setting.Color then
						setting.Color = KingLib:ColorUnpack(val)
...
--[[	local uptimeParagraph = element:Paragraph({
	...
	function ldata:InitTab(Title)
		...
		if ldata.first then
			...
			tweenservice:Create(Tab.Title, TweenInfo.new(2, Enum.EasingStyle.Exponential), { TextTransparency = 0 }):Play()
			tweenservice:Create(Tab.indicator, TweenInfo.new(2, Enum.EasingStyle.Exponential), { BackgroundColor3 = KingLib.theme.Accent }):Play()
		...
		local function ApplyTabStyle(tabButton, isSelected)
			...
			local targetColor = isSelected and KingLib.theme.Accent or Color3.fromRGB(29, 29, 29)
		...
		function InitElement:Toggle(Toggle)
			...
			if KingLib.ConfigEnabled then
				if ToggleData.Flag then
					KingLib.Flags[ToggleData.Flag] = ToggleData
				end
			end
		...
		function InitElement:CreateSlider(Slider)
			...
			for _, Options in ipairs(SliderData.Sliders) do
				...
				--	KingLib:replayLoadTweens(Slider.slide.slideframe)
				...
				KingLib:AddConnection(userinput.InputEnded, function(input, processed)
					if input.UserInputType == Enum.UserInputType.MouseButton1 then
						dragging = false
						if Slider and Slider:FindFirstChild("Title") then
							tweenservice:Create(Slider.Title, TweenInfo.new( 0.5, Enum.EasingStyle.Exponential ), { TextTransparency = 0.6 }):Play()
				...
				if KingLib.ConfigEnabled then
					if Options.Flag then
						KingLib.Flags[Options.Flag] = Options
					end
				end
		...
		function InitElement:ColorPicker(ColorPicker)
			...
			if KingLib.ConfigEnabled then
				if ColorPickerData.Flag then
					KingLib.Flags[ColorPickerData.Flag] = ColorPickerData
				end
			end
		...
		function InitElement:Keybind(Keybind)
			...
			-- Main input handler
			KingLib:AddConnection(userinput.InputBegan, function(input)
				if input.UserInputType ~= Enum.UserInputType.Keyboard then return end
		...
		function KingLib:LoadSaveConfig()
			...
			if KingLib.ConfigEnabled then
				...
				elseif not success then
					warn("KingLib Configurations Error ã€¡ " .. tostring(result))
				end
			end
		end
